...
namespace BlackCat\Database\Packages\EncryptionPolicyBindings\Service;

use BlackCat\Core\Database;
use BlackCat\Database\Packages\EncryptionPolicyBindings\Dto\EncryptionPolicyBindingDto;
use BlackCat\Database\Packages\EncryptionPolicyBindings\Mapper\EncryptionPolicyBindingDtoMapper;
 ...
 */
final class EncryptionPolicyBindingsAggregateService
{
    use ServiceHelpers;
    ...
    public function __construct(
        private Database $db, private EncryptionPolicyBindingRepository $encryptionPolicyBindingRepo
    ) {
        // Repository autowiring (generated)
        $this->repositories = [
  'encryption_policy_bindings' => $this->encryptionPolicyBindingRepo
];
    }
    ...
    protected function withRowLock(callable $locker, int|string|array $id, callable $fetch, string $mode = 'wait'): mixed
    {
        ...
        return $this->txn(function () use ($locker, $id, $fetch, $mode) {
            ...
            if (!$row) {
                ...
                throw new \BlackCat\Database\Packages\EncryptionPolicyBindings\ModuleException("Record {$idStr} not found for locking.");
            }

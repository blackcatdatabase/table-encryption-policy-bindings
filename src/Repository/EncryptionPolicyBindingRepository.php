...
use BlackCat\Database\Packages\EncryptionPolicyBindings\Repository\EncryptionPolicyBindingRepositoryInterface;
...
class EncryptionPolicyBindingRepository implements EncryptionPolicyBindingRepositoryInterface, RepoContract, KeysetRepoContract
{
use OrderByTools, PkTools, RepositoryHelpers;

    /** @var mixed literal token for upsert keys (array or empty). */
    private mixed $tokenUpsertKeys = [ 'entity_table', 'field_name', 'effective_from' ];
    ...
    public function upsertMany(array $rows): int {
        ...
        if ($helperKeys && class_exists(\BlackCat\Database\Support\BulkUpsertHelper::class)) {
          $bulk = new \BlackCat\Database\Support\BulkUpsertHelper($this->db, \BlackCat\Database\Packages\EncryptionPolicyBindings\Definitions::class);
          $bulk->upsertMany($rows, $helperKeys, [ 'policy_id', 'notes' ]);
          return count($rows);
        }

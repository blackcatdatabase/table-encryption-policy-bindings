...
namespace BlackCat\Database\Packages\EncryptionPolicyBindings\Repository;

use BlackCat\Core\Database as Database;              // e.g. BlackCat\Core\Database
use BlackCat\Database\Packages\EncryptionPolicyBindings\Definitions;
use BlackCat\Database\Packages\EncryptionPolicyBindings\Criteria;
use BlackCat\Database\Packages\EncryptionPolicyBindings\Dto\EncryptionPolicyBindingDto as Dto;
use BlackCat\Database\Packages\EncryptionPolicyBindings\Mapper\EncryptionPolicyBindingDtoMapper as RowMapper;
...
final class EncryptionPolicyBindingRepository implements RepoContract, KeysetRepoContract
{
     ...
     */
    protected function def(): string { return \BlackCat\Database\Packages\EncryptionPolicyBindings\Definitions::class; }
    ...
    public function upsertMany(array $rows): int {
        ...
        if (class_exists(\BlackCat\Database\BulkUpsertRepository::class) && [ 'entity_table', 'field_name', 'effective_from' ] !== []) {
            $bulk = new \BlackCat\Database\BulkUpsertRepository($this->db, \BlackCat\Database\Packages\EncryptionPolicyBindings\Definitions::table());
            return $bulk->upsertMany($rows, [ 'entity_table', 'field_name', 'effective_from' ], [ 'policy_id', 'notes' ]);
        }
    ...
    */
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\EncryptionPolicyBindings\Dto\EncryptionPolicyBindingDto|null */
    public function getByEntityTableAndFieldNameAndEffectiveFrom(string $entityTable, string $fieldName, \DateTimeInterface|string $effectiveFrom, bool $asDto = false): array|\BlackCat\Database\Packages\EncryptionPolicyBindings\Dto\EncryptionPolicyBindingDto|null {
        $row = $this->getByUnique([ 'entity_table' => $entityTable, 'field_name' => $fieldName, 'effective_from' => $effectiveFrom ]);
        if (!$asDto || !$row) return $row;
        return \BlackCat\Database\Packages\EncryptionPolicyBindings\Mapper\EncryptionPolicyBindingDtoMapper::fromRow($row);
    }
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\EncryptionPolicyBindings\Dto\EncryptionPolicyBindingDto|null */
    public function getById(int $id, bool $asDto = false): array|\BlackCat\Database\Packages\EncryptionPolicyBindings\Dto\EncryptionPolicyBindingDto|null {
